...
def S(beta, lambda_constant):
    ...
    S_beta = np.zeros(beta.shape[0])
    for i in range(beta.shape[0]):
        if beta[i] > lambda_constant:
            S_beta[i] = beta[i] - lambda_constant
        elif beta[i] >= -lambda_constant and beta[i] <= lambda_constant:
            S_beta[i] = 0
