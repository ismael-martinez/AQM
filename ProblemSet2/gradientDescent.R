...
# Out: x* (float)
gradient.descent <- function(func, alpha, b,x,y){
  b.prev <- b
  repeat{
    mv <- single.gradient.descent(func,alpha,b.prev,x,y)
    b <- b.prev + mv
    if(all(abs(func(b,x,y)-func(b.prev,x,y)) < 0.0001)){ #barely changed from prev iter.
      break
    }else{
      b.prev <- b
    }
...
single.gradient.descent <- function(func, alpha, b,x,y){
  grad <- partial.derivative(func,b,x,y)
  ...
  # Out: p (float vector)
  partial.derivative <- function(func,b,x,y){
    ...
    p <- (func(b.right,x,y) - func(b.left,x,y))/(2*epsilon) #vector of partial derivatives
    p
